...
### 诊断工具已上线 (`mnemos doctor`)
1. **全项检查**：涵盖 Git 环境、文件完整性、配置合法性及 Skill 同步状态。
2. **健康报告**：提供清晰的 [✓] / [✗] 指示，并给出修复建议（如重新运行 `mnemos init`）。

### 2026-02-06 技术里程碑
- **智能化总结**：支持 Conventional Commits 分组与变动热点分析。
- **搜索功能**：跨文件全文本检索。
- **动态配置**：引入 `.mnemos.toml` 管理全局设置。
- **安全保障**：修复退出码逻辑，引入文件级安全写入流程。
- **自动化提醒**：短期记忆容量预警。
- **健康诊断**：一键检查项目运行环境。

*追加于: 2026-02-06 17:44*
