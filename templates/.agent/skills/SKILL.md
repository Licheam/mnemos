...
```bash
# 替换 section 内容
mnemos write --section "架构决策" --content "### 使用 SQLite\n选择 SQLite 作为本地数据库..."

# 追加到 section
mnemos write --section "重要约束与注意事项" --content "### 新发现的坑\n..." --append
```
...
**触发时机**: 每周一次，或短期记忆过大时。返回旧内容供总结后写入长期记忆。

## 会话启动流程

每次开启新会话时：

1. 运行 `mnemos update` 更新短期记忆
2. 运行 `mnemos show -t long` 读取长期记忆作为上下文
